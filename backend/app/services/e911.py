...
#: Carriers we register 911 addresses with (P44e added SignalWire).
SUPPORTED_CARRIERS = ("telnyx", "signalwire")
#: States that let a number make ordinary outbound calls (P44e call gate).
CALLABLE_STATES = ("active", "provisioning", "pending")
...
def status_of(number: OrgNumber) -> dict:
    ...
    return {
        ...
        or ("unsupported" if number.carrier not in SUPPORTED_CARRIERS else "missing"),
...
def _signalwire_base(settings) -> tuple[str, tuple[str, str]]:
    host = (settings.signalwire_space_url or "").strip()
    host = host.removeprefix("https://").removeprefix("http://").strip("/")
    token = settings.signalwire_api_token.get_secret_value().strip()
    if not (host and settings.signalwire_project_id and token):
        raise FeatureUnavailableError("Emergency addresses cannot be registered right now.")
    return f"https://{host}/api/relay/rest", (settings.signalwire_project_id, token)


async def _signalwire(method: str, path: str, settings, *, client=None, **kwargs) -> dict:
    """SignalWire REST (relay) call. Errors surface like the Telnyx ones."""
    base, auth = _signalwire_base(settings)
    owned = client is None
    client = client or httpx.AsyncClient(timeout=15)
    try:
        resp = await client.request(method, f"{base}{path}", auth=auth, **kwargs)
    except httpx.HTTPError as exc:
        raise FeatureUnavailableError("The emergency-calling provider is unreachable.") from exc
    finally:
        if owned:
    ...
    if resp.status_code >= 400:
        ...
        detail = (
            (first.get("detail") or first.get("title")) if isinstance(first, dict) else first
        ) or str(resp.status_code)
        raise ValidationFailedError(f"Emergency address was refused: {detail}")
    return body if isinstance(body, dict) else {}


async def _enable_signalwire(session, settings, number: OrgNumber, address, *, client=None):
    """Register the address with SignalWire on first use, then bind it to the number."""
    if not address.signalwire_address_id:
        house, _, street = address.street_address.strip().partition(" ")
        if not any(ch.isdigit() for ch in house):
            house, street = "", address.street_address.strip()
        first, _, last = address.name.partition(" ")
        payload = {
            "label": address.name[:64],
            "country": address.country_code,
            "first_name": first or address.name,
            "last_name": last or first or address.name,
            "street_number": house,
            "street_name": street,
        ...
        if address.extended_address:
            ...
            payload["address_number"] = address.extended_address
        created = await _signalwire("POST", "/addresses", settings, client=client, json=payload)
        if not created.get("id"):
            raise ValidationFailedError("The carrier did not register the emergency address.")
        address.signalwire_address_id = str(created["id"])
    if not number.provider_ref:
        raise FeatureUnavailableError("This number has no carrier id yet.")
    await _signalwire(
        "POST",
        f"/phone_numbers/{number.provider_ref}/e911_address",
        settings,
        client=client,
        json={"e911_address_id": address.signalwire_address_id},
    )
    _set(number, status="provisioning", signalwire_bound=True, detail=None)
...
async def enable(session, settings, number: OrgNumber, address: EmergencyAddress, *, client=None):
    ...
    try:
        if number.carrier == "signalwire":
            await _enable_signalwire(session, settings, number, address, client=client)
...
async def refresh(session, settings, number: OrgNumber, *, client=None) -> None:
    ...
    e911 = (number.provisioning or {}).get("e911") or {}
    if number.carrier == "signalwire":
        if not (e911.get("signalwire_bound") and number.provider_ref):
            return
        row = await _signalwire(
            "GET", f"/phone_numbers/{number.provider_ref}", settings, client=client
        )
        status = {"active": "active", "pending": "provisioning", "failed": "failed"}.get(
            str(row.get("e911_status") or "")
        )
        if status:
...
async def enable_for_purchase(session, settings, purchase, *, client=None) -> None:
    ...
    for number_id in ids:
        ...
        if (
            number is not None
            and number.carrier in SUPPORTED_CARRIERS
            and number.status != "released"
        ):
...
async def tick(session_factory, settings, *, client=None) -> dict[str, int]:
    ...
    async with session_factory() as session:
        rows = (
            await session.execute(
                ...
                .where(
                    OrgNumber.carrier.in_(SUPPORTED_CARRIERS), OrgNumber.status == "active"
                )
                .execution_options(**{ALLOW_UNSCOPED_KEY: True})
            )
        ).all()
        first_address = {}
        for org_id, address_id in (
            await session.execute(
                sa.select(EmergencyAddress.org_id, EmergencyAddress.id)
    ...
    for number_id, org_id in unassigned:
        async with session_factory() as session:
            ...
            number = await session.get(OrgNumber, number_id)
            address = await session.get(EmergencyAddress, first_address[org_id])
            try:
                if number is not None and address is not None:
                    await enable(session, settings, number, address, client=client)
                    counts["assigned"] += 1
            except Exception:
                log.exception("e911_auto_assign_failed", number_id=str(number_id))
    for number_id, org_id in todo:
        async with session_factory() as session:
            ...
            try:
                bound = (
                    e911.get("signalwire_bound")
                    if number.carrier == "signalwire"
                    else e911.get("telnyx_phone_id")
                )
                if e911.get("status") == "failed" or not bound:
...
def grace_deadline(settings, number: OrgNumber) -> datetime:
    """When calls from ``number`` start needing a working 911 address: the enforcement
    start plus the grace days, or the number's purchase plus the grace days if later."""
    days = int(getattr(settings, "e911_grace_days", 7))
    try:
        start = datetime.fromisoformat(str(settings.e911_enforcement_start)).replace(
            tzinfo=timezone.utc
        )
    except (AttributeError, ValueError):
        start = datetime(2026, 9, 26, tzinfo=timezone.utc)
    deadline = start + timedelta(days=days)
    bought = number.purchased_at or number.created_at
    if bought is not None:
        if bought.tzinfo is None:
            bought = bought.replace(tzinfo=timezone.utc)
        deadline = max(deadline, bought + timedelta(days=days))
    return deadline


async def require_e911(session, settings, org_id: uuid.UUID, from_e164: str, to: str) -> None:
    """P44e: refuse an ordinary outbound call from a number whose 911 address is not set up
    (after its grace period). Emergency calls are never refused here."""
    if not getattr(settings, "e911_enforced", True):
        return
    if is_emergency((to or "").removeprefix("+1")):
        return
    number = (
        await session.execute(
