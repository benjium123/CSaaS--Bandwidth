 ...
 */
import { CALLS_PER_NUMBER, COVERAGE, DAILY_OUTBOUND_PER_NUMBER, MINUTES_PER_USER, PRICED_PLANS, RATES, cents, money, planByCode } from "./pricing.config";
...
export const FAQS: Faq[] = [
  ...
  // Plans and billing
  { id: "plans", topic: "Plans and billing", q: "What plans do you offer?", a: `${planLine}. Need more? Extra users and numbers can be added to any plan, and bigger teams can talk to us about a Custom plan. Every user brings ${MINUTES_PER_USER} call minutes a month, shared by the team; after that, calls and texts are pay as you go.`, keywords: ["plans", "pricing", "price", "how much", "cost", "tiers", "packages"] },
  { id: "per-number", topic: "Plans and billing", q: "How does pricing work?", a: `Each plan is one monthly price that includes a set number of users and phone numbers. Add users or numbers whenever you need them: on Starter and Team an extra user is ${money(starter.extraUser ?? 0)} and an extra number ${money(starter.extraNumber ?? 0)}; on Business an extra user is ${money(business.extraUser ?? 0)} and an extra number ${money(business.extraNumber ?? 0)}. Each user adds ${MINUTES_PER_USER} call minutes a month to a shared pool.`, keywords: ["per number", "per user", "per seat", "per line", "how does pricing", "add user", "add number"] },
