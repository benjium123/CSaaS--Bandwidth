 ...
 */
import { CALLS_PER_NUMBER, COVERAGE, DAILY_OUTBOUND_PER_NUMBER, PRICED_PLANS, perBill, RATES, cents, money, planByCode } from "./pricing.config";
...
export const FAQS: Faq[] = [
  ...
  // Plans and billing
  { id: "plans", topic: "Plans and billing", q: "What plans do you offer?", a: `${planLine}. Need more? Extra users and numbers can be added to any plan, and bigger teams can talk to us about a Custom plan. Team includes ${planByCode("team").minutes} call minutes a month and Business ${(planByCode("business").minutes ?? 0).toLocaleString("en-US")}, shared by the team; Starter and anything past the pool is pay as you go.`, keywords: ["plans", "pricing", "price", "how much", "cost", "tiers", "packages"] },
  { id: "per-number", topic: "Plans and billing", q: "How does pricing work?", a: `Each plan is one monthly price that includes a set number of users and phone numbers. Add users or numbers whenever you need them: on Starter and Team an extra user is ${money(starter.extraUser ?? 0)} and an extra number ${money(starter.extraNumber ?? 0)}; on Business an extra user is ${money(business.extraUser ?? 0)} and an extra number ${money(business.extraNumber ?? 0)}. Adding users does not change the plan's shared minute pool.`, keywords: ["per number", "per user", "per seat", "per line", "how does pricing", "add user", "add number"] },
  ...
  { id: "yearly", topic: "Plans and billing", q: "Can I pay yearly?", a: `Yes. Pay yearly and get 2 months free: Starter is ${money(perBill(planByCode("starter").price ?? 0, "year"))} a year, Team ${money(perBill(planByCode("team").price ?? 0, "year"))} and Business ${money(perBill(planByCode("business").price ?? 0, "year"))}. Extra users and numbers on a yearly plan are billed yearly too.`, keywords: ["yearly", "annual", "annually", "year", "discount", "save", "2 months free"] },
  { id: "user-limits", topic: "Plans and billing", q: "How many users can each plan have?", a: `Starter goes up to ${planByCode("starter").maxUsers} users, Team up to ${planByCode("team").maxUsers}, and Business has no limit. Need more on a smaller plan? Move up at any time and keep everyone and every number.`, keywords: ["how many users", "user limit", "max users", "maximum", "seats", "cap", "limit users"] },
  { id: "overage", topic: "Plans and billing", q: "Are minutes included?", a: `On Team and Business, yes: Team includes ${planByCode("team").minutes} call minutes a month and Business ${(planByCode("business").minutes ?? 0).toLocaleString("en-US")}, in one pool the whole team shares. Starter is pay as you go. Past the pool, calls are ${cents(RATES.minute)} a minute and texts ${cents(RATES.text)}, from a prepaid balance. Heavy users can buy bundles, like ${RATES.minuteBundle.units.toLocaleString()} minutes for ${money(RATES.minuteBundle.price)}.`, keywords: ["minutes included", "free minutes", "overage", "run out", "more minutes", "extra minutes", "limit", "pool"] },
