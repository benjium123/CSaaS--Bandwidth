...
## Discovered while building the prepaid telephony gate (Fable, 2026-09-11)

| ID | Issue | Found | Recipe |
|---|---|---|---|
| D69 | P24's `ai_usage.credits_tick` (stale-hold release, low-balance warnings, auto-recharge, pausing campaigns when empty) was written and fixed, but **never called from the sweeper** - none of it ever ran in production. | Fable, prepaid gate build | ✅ Wired into `services/sweeper.py` with the telephony tick. Campaign pause now also requires a gate to actually be on (AI enforcement or the org's `telephony_prepaid`), so a shadow-mode org is never paused. |
| D70 | `GET /billing/summary` returns `available_micros = balance - outstanding_reserves`, but a reserve is already a negative ledger entry, so `balance()` is net of holds - holds are subtracted twice and a customer sees less available than they have while a call is held. Display only; the gates use the true balance. | Fable, prepaid gate build | Return `available_micros = balance` (or drop it); keep `reserved_micros` as information. Fix with the P37c billing UI pass. |
| D71 | `tests/test_agent_tools.py` takes ~8 min 20 s for 22 tests (500 s in a per-file run, `-x`); `test_appointment_booking_worker_auth_enforced` and neighbours stall for minutes each before passing - most likely a real network wait (DNS / connect timeout) on this Windows box rather than a logic error. A whole-suite `python -m pytest` run sat 2 hours at 23 s of CPU on 2026-09-11 before being killed, so the earlier "full suite green" claim for the prepaid gate was never a pytest exit code. | Fable, prepaid gate suite run, 2026-09-11 | Open - find the blocking call with `-o faulthandler_timeout=45 -v` on that file (the first run of this was cut off by an external timeout before the dump). Until fixed, run the suite per file with a 600 s cap (`scratchpad/run_chunked.py` pattern) and read `SUITE_EXIT`, never a wrapper's exit code. |

## Discovered during P37a managed telephony (Opus supervisor + Fable, 2026-09-11)

| ID | Issue | Found | Recipe |
|---|---|---|---|
| D72 | Which Ed25519 public key signs a managed SUB-ACCOUNT's webhooks is unverified; provisioning stores the platform's `TELNYX_PUBLIC_KEY` on the org's provider row. If wrong, a managed org can send but every inbound webhook is rejected (fails closed, loud). Also unverified until the first live run: `GET /v2/managed_accounts/{id}` carrying `api_key` (handled: stops with a plain sentence, resumes after Telnyx enables access), `data.id` shapes on the two creates, plain Bearer auth with the sub-account key. | P37a supervisor | Confirm on the first live sub-account before any customer relies on inbound; if a per-account key exists, read it in step 4 of `services/telephony_provisioning.py`. |
| D73 | `persist_ordered_number` (OrgNumber + Inbox in one transaction, release-on-conflict) was factored out of `routes/numbers.py::order` and is imported by `services/telephony_provisioning.py` - a service importing a route module. Behaviour is unchanged. | P37a supervisor | Move the helper into a `services/numbers.py` module when a third caller appears. |
| D74 | Re-running `POST /telephony/setup` on an active org (the "Repair setup" path) re-PATCHes and re-probes the provider row, which sets it `unverified` for the duration of one Telnyx GET. A send racing that window could pick a registry without the row. | P37a supervisor | Skip step 4 when the row is already active with the same credentials, or probe before writing. |
| D75 | Fable review closed: `POST /telephony/setup` on a `suspended` account went provisioning -> skipped every stored step -> `active` (a self-service un-suspend). Now refused with a plain sentence; regression test `test_a_suspended_account_cannot_reactivate_itself_through_setup`. | Fable, P37a sign-off | ✅ Fixed before commit. |
